...
# --- AJOUT / RETRAIT PN, PIECES, COUPES ---
...
async def add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_creator(update):
        ...
        return
    if not update.message.reply_to_message:
        await update.message.reply_text("Tu dois répondre à un message du joueur cible.")
        return

    args = context.args
    if len(args) < 2:
        await update.message.reply_text("Usage : /add [quantité] [pn/pièces/coupes]")
        return

    try:
        quantite = int(args[0])
        if quantite <= 0:
            await update.message.reply_text("La quantité doit être un entier positif.")
            return
    except:
        await update.message.reply_text("La quantité doit être un nombre entier.")
        return

    ressource = args[1].lower()
    if ressource not in ["pn", "pieces", "pièces", "coupes"]:
        await update.message.reply_text("La ressource doit être 'pn', 'pièces' ou 'coupes'.")
        return

    player = get_player(update.message.reply_to_message.from_user)

    if ressource == "pn":
        player["pn"] += quantite
        leveled = check_and_update_level(player)
        if leveled:
            await update.message.reply_text(
                f"{update.message.reply_to_message.from_user.first_name} a gagné un niveau ! Nouveau niveau : {player['niveau']}"
            )
        else:
            await update.message.reply_text(
                f"{quantite} PN ajoutés à {update.message.reply_to_message.from_user.first_name}."
            )
    elif ressource in ["pieces", "pièces"]:
        player["pieces"] += quantite
        await update.message.reply_text(
            f"{quantite} pièces ajoutées à {update.message.reply_to_message.from_user.first_name}."
        )
    elif ressource == "coupes":
        player["coupes"] += quantite
        await update.message.reply_text(
            f"{quantite} coupes ajoutées à {update.message.reply_to_message.from_user.first_name}."
        )
...
async def delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("Usage : /delete [quantité] [pn/pièces/coupes]")
        return
    ...
    if ressource == "pn":
        if player["pn"] < quantite:
            await update.message.reply_text("Le joueur n'a pas assez de PN.")
            return
        player["pn"] -= quantite
        # Vérification régression niveau
        leveled = check_and_update_level(player)
        if leveled:
            await update.message.reply_text(
                f"{update.message.reply_to_message.from_user.first_name} a perdu un niveau... Nouveau niveau : {player['niveau']}"
            )
        else:
            await update.message.reply_text(
                f"{quantite} PN retirés à {update.message.reply_to_message.from_user.first_name}."
            )
    elif ressource in ["pieces", "pièces"]:
        if player["pieces"] < quantite:
            await update.message.reply_text("Le joueur n'a pas assez de pièces.")
            return
        player["pieces"] -= quantite
        await update.message.reply_text(
            f"{quantite} pièces retirées à {update.message.reply_to_message.from_user.first_name}."
        )
    elif ressource == "coupes":
        if player["coupes"] < quantite:
            await update.message.reply_text("Le joueur n'a pas assez de coupes.")
            return
        player["coupes"] -= quantite
        await update.message.reply_text(
            f"{quantite} coupes retirées à {update.message.reply_to_message.from_user.first_name}."
        )
...
# Lance le petit serveur HTTP en parallèle pour Render
